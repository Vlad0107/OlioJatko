...
Henkilo::Henkilo(const Henkilo& aHlo) : ika( aHlo.ika), nimi( aHlo.nimi ), osoite( new Osoite( *(aHlo.osoite) ) )
{
...
Osoite* Henkilo::getOsoitePtr() const
{
	return osoite;
}
...
void Henkilo::setOsoite(const Osoite & aOsoite)
{
	if (osoite) { // osoite on luotu ja olemassa => päivitetään osoite
		*osoite = aOsoite; // kopioidaan uudet arvot vanhaan osoitteeseen
		...
		osoite = new Osoite(aOsoite);	// luodaan henkilölle osoite, johon kopioidaan							// aOsoite -olion arvot (kopiorakentaja)
	}
}

void Henkilo::setOsoitePtr(Osoite* aOsoite)
{ 
